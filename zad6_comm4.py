...
def load_coingecko_list():
    ...
    if os.path.exists(CACHE_FILE):
        try:
            with open(CACHE_FILE, 'r', encoding='utf-8') as f:
                cache = json.load(f)
                # Проверяем, что кэш не старше 1 часа (3600 секунд)
                if time.time() - cache.get('timestamp', 0) < 3600:
                    # Проверяем, что данные для CoinGecko существуют
    ...
    # Если кэша нет или он устарел, загружаем с CoinGecko
    try:
        # Задержка перед запросом, чтобы не превысить лимит
        time.sleep(1)
        ...
        # Если запрос не удался, пробуем использовать устаревший кэш
        if os.path.exists(CACHE_FILE):
            try:
                with open(CACHE_FILE, 'r', encoding='utf-8') as f:
                    cache = json.load(f)
                    if 'coingecko' in cache:
                        ...
                        mb.showwarning("Внимание",
                                       f"Используются кэшированные данные (могут быть устаревшими).\n"
                                       f"Ошибка: {str(e)}")
                        return True
            except:
                pass
