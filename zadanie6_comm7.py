...
# Доступные источники данных (БЕЗ CoinMarketCap)
DATA_SOURCES = ["CoinGecko", "CoinCap", "CoinPaprika", "Binance", "Kraken"]
...
def get_coingecko_coin_id(symbol):
    ...
    if os.path.exists(COINS_LIST_CACHE):
        try:
            ...
            mb.showerror("Ошибка - кэш недействителен:", f"CoinGecko: {e}")
    ...
    try:
        ...
        mb.showerror("Ошибка получения списка монет:", f"CoinGecko: {e}")
...
def load_coingecko_list():
    ...
    if os.path.exists(CACHE_FILE):
        try:
            ...
            mb.showerror("Ошибка наличия кэша", f"CoinGecko: {e}")
    ...
    try:
        ...
        if os.path.exists(CACHE_FILE):
            try:
                ...
                mb.showerror("Ошибка при использовании уставревшего кэша", f"CoinGecko: {e}")
...
def show_info():
    mb.showinfo("О программе", "Источники: \n"
                               "CoinGecko (не требует ключа, но с задержкой 3 сек);\n"
                               ...
                               "Особенности API: \n"
                               "CoinGecko: \n"
                               " Список: https://api.coingecko.com/api/v3/coins/markets\n"
                               " Цена: https://api.coingecko.com/api/v3/simple/price\n"
                               "CoinCap: \n"
