...
# --- УНИВЕРСАЛЬНАЯ ФУНКЦИЯ СОХРАНЕНИЯ ГРАФИКА ---

def save_chart(fig, default_filename, title="Успех", message="График сохранен как:"):
    """
    Универсальная функция сохранения графика

    Параметры:
    - fig: объект Figure из matplotlib
    - default_filename: имя файла по умолчанию (без расширения)
...
def plot_chart(historical_data, crypto_symbol, crypto_name, days):
    ...
    # Кнопка сохранения - используем универсальную функцию
    Button(chart_window, text="Сохранить график",
           command=lambda: save_chart(
               fig,
               f"{crypto_symbol}_{days}days_{datetime.now().strftime('%Y%m%d')}.png",
               "Успех",
               "График сохранен как:"
           ),
    ...
    def get_and_plot():
        try:
            ...
            Button(chart_window, text="Сохранить график",
                   command=lambda: save_chart(
                       fig,
                       f"compare_{crypto1_code}_{crypto2_code}_{days}days_{datetime.now().strftime('%Y%m%d')}.png",
                       "Успех",
                       "График сохранен как:"
                   ),
                   bg="#4CAF50", fg="white", padx=20, pady=5).pack(pady=10)
